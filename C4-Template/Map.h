...
public:
	...
	sf::Vector2i getTilePosFromIndex(int N) {
		return sf::Vector2i(N % mapWidth, std::floor(N / mapWidth));
	}
	...
	//Find a wall's location relative to unit
	sf::Vector2f wallLocation(Actor& actor);

	//Use astar[1] to get the dir for enemy pathfinding
	std::vector<gridcoord> astar(gridcoord start, gridcoord end);
};